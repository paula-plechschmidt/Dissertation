...
movingAverageMomentum4 <- function(df, k, beta, gamma){
  ...
  for (time in df_fav$timeToStart){
    ...
    if (count > k){
      ...
      #print(df_fav$bckPrc1[(count-k+1):count])
      ...
      #If we have an active trade..
        ...
        if(backThenLay){
          ...
          # If we didn't cover the spread
            ...
            if(currentLayOdds > df_ma[df_ma$timeToStart == tradeStart,]$layMa[1] + gamma*averageSpread){
        ...
        ### LAY THEN BACK ###
          ...
          if (spreadCovered){
            ...
            if(currentBckOdds < df_ma[df_ma$timeToStart == tradeStart,]$bckMa[1] - gamma*averageSpread){
              #print("Odds too far out of line")
              #print(df_fav[df_fav$timeToStart == time,]$callTime_GMT)
              endOdds <- df_fav$bckPrc1[count]
              #print(endOdds)
...
races <- sample(c(1:509,511:865,867:889), 150, replace=F)#DONT RUN AGAIN
...
for (i in theRaces){
  print(count)
  print(i)
  profitsMA_015_10 <- c(profitsMA_015_10, movingAverageMomentum4(data[[i]], 20, 0.15, 1))
  print(mean(profitsMA_015_10))
  count = count + 1
}
 
profits <- list()
highLevelCount <- 1
for (j in c(0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65)){
  profitsMA_015_10 <- c()
  count = 1
  for (i in theRaces){
    print(paste0("Beta is ", j))
    print(count)
    print(i)
    profitsMA_015_10 <- c(profitsMA_015_10, movingAverageMomentum4(data[[i]], 20, j, 2.8))
    print(mean(profitsMA_015_10))
    count = count + 1
  }
  profits[[highLevelCount]] <- profitsMA_015_10 
  highLevelCount = highLevelCount + 1
}

write.csv(profits[[5]],file="profitsMA_065_10.csv",row.names=F)
ggplot(data.frame(profitsMA_23[[3]]), aes(x=profitsMA_23[[3]])) + geom_histogram() +
  ggtitle("Histogram of Profits with Adjusted Moving Average Trading Method") + 
  xlab("Profits") #+ xlim(-15,10)


profitsMA_10 <- profits
profitsMA_16 <- profits
profitsMA_20 <- profits
profitsMA_23 <- profits
