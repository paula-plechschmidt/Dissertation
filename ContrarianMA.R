contrarianMA1 <- function(df, k, beta, gamma){
  ...
  for (time in df_fav$timeToStart){
    ...
    if (count > k){
      ...
      while(!(timeMin120 %in% df_ma$timeToStart)){
        timeMin120 <- timeMin120 + 1
      }
      ...
      if(!tradeActive & (time >= marketOpen + 120)){
        #... and the moving average of the backing prices is increasing over two minutes
        if((newRow[3] > (df_ma[df_ma$timeToStart == timeMin120,]$bckMa[1] + 2*averageSpread))){
          #...we enter a trade
          print("---------------------------------------------------------------")
        ...
        #... and the moving average of the laying prices is decreasing over two minutes
        else if((newRow[4] < (df_ma[df_ma$timeToStart == (timeMin120),]$layMa[1] - 2*averageSpread))){
          #...we enter a trade
          print("--------------------------------------------------------------- ")
